    ...
    >
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-[11px] font-mono" style={{ color: statusColor }}>
          {v.status === "confirmed" ? "CONFIRMED" : v.status === "confirming" ? "VERIFYING" : "CANDIDATE"}
        </span>
      </div>
      <p className="text-xs text-white/90 mb-2">{v.title}</p>
...
export function ThreatPanel() {
  return (
    <div className="glass rounded-xl p-5 h-full flex flex-col overflow-hidden">
      <h3 className="text-[11px] uppercase tracking-[0.2em] text-white/60 mb-4 font-mono">THREAT ANALYSIS</h3>
      <div className="flex-1 min-h-0 overflow-y-auto">
        <p className="text-[11px] text-white/50 mb-3 font-mono">Vulnerabilities</p>
        {VULNS.map((v, i) => (
          <VulnCard key={v.id} v={v} i={i} />
        ))}
      </div>
      <div className="rounded-lg border border-white/10 p-3 bg-black/30 mt-3">
        <p className="text-[11px] text-white/50 mb-2 font-mono">Code View</p>
